...
v1.0.4 Update:
更新内容：修复部分高分辨率色彩匹配节点逻辑，添加了视频色彩匹配节点，现在支持视频的色彩匹配。

v1.0.3 Update:
更新内容
预设系统升级（Presets Update）：对预设功能进行了全面更新与优化，提升了使用体验与兼容性。
节点命名规范化：将原有的 Latent Size 节点正式更名为 Resolution（分辨率）。
升级提示
节点更名说明：Resolution 节点的功能与原有 Latent Size 保持一致，仅做命名上的语义优化，以更准确地反映其控制图像尺寸的实际用途。
工作流兼容性：如果你之前的工作流中使用了 Latent Size 节点，在加载旧版工作流时可能会提示节点缺失。只需在 Manager 中搜索并替换为新的 Resolution 节点即可。
